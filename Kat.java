...
/**
 * @author Adam Steinberg of Digidemic, LLC
 *
...
public class Kat {
     ...
     * With this call, the method gets the application's package name, if in debug, and requests to the device for the "WRITE_EXTERNAL_STORAGE" permission if needed (only if application version is in debug and context be an Activity instance to request permission for devices API 23 and higher).
     ...
     *
     * NOTE:
     * IF CLASS IMPORTED OR COPY/PASTED INTO YOUR PROJECT, ADD IN THE FOLLOWING PERMISSION TO YOUR MANIFEST: <uses-permission android:name="android.permission.WRITE_EXTERNAL_STORAGE" />
     * If .aar library was imported into your project then this permission does not need to be added.
     *
     ...
     *
     * @param context Context instance from the application to determine the application's package name and if the application version is in debug. Activity must be passed in to request the "WRITE_EXTERNAL_STORAGE" to be granted for devices API 23 and higher.
     * @param addEntriesIntoSubdirectoryCreatedToday If true will add all entries from Kat.scan calls into a subfolder of the current day. Ex: "/storage/emulated/0/KatScan_com.digidemic.katscanexamples/2018-09-30/KatScan_log.txt" rather than "/storage/emulated/0/KatScan_com.digidemic.katscanexamples/KatScan_log.txt"
     * @return true if successfully called without error
     */
    public static boolean setup(Context context, boolean addEntriesIntoSubdirectoryCreatedToday){
        try{
            if(!setupComplete && context != null) {
                ...
                Config.File.addEntriesIntoSubdirectoryCreatedToday = addEntriesIntoSubdirectoryCreatedToday;        //If Kat.scan() entries should be added into a subdirectory of the current date
    ...
    private static void waitForUserPermissionRequestToFinish(final Context CONTEXT){    //Instead of timer, could use the normal permission callback listener but do not want to add listeners to KatScan as it could interfere negatively with the rest of the application and its permission callback listeners.
        try {
    ...
    public static class Config{
        public static class File{
            ...
            public static boolean addEntriesIntoSubdirectoryCreatedToday = false;   //To have each Kat.scan() entry written into a subfolder of the current day within "mainDirectoryName". | Default: false - HOWEVER: an overloaded Kat.setup() method can be called to assign this value true.
        ...
        public static class InternalErrors{
            ...
            public static boolean showTheSetupErrorAsLogOnceIfNeededRegardlessIfDebugging = true;   //If this error has not yet occurred or has not been displayed to the user's console yet. | Default: true
        ...
        /**
         * If KatScan detected that the application is running in debug mode, not release mode
         */
